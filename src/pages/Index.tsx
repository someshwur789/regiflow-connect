...
const Index = () => {
  const [selectedEvent, setSelectedEvent] = useState<EventName>('Paper Showcase');
        ...
        <div className="mb-8">
          <Tabs value={selectedEvent} onValueChange={value => setSelectedEvent(value as EventName)}>
            <div className="max-w-4xl mx-auto">
              ...
              <TabsList className="hidden md:grid w-full grid-cols-5 mb-8 bg-muted/50">
                {EVENTS.map(event => {
              ...
              {/* Mobile: Two rows - 3 tabs on top, 2 tabs below */}
              <div className="md:hidden space-y-3">
                ...
                <TabsList className="grid w-full grid-cols-3 bg-muted/50">
                  ...
                  const config = EVENT_CONFIGS.find(c => c.name === event)!;
                  const isFull = isEventFull(event);
                  return <TabsTrigger key={event} value={event} disabled={isFull} className="text-[11px] leading-tight relative flex flex-col gap-0.5 py-1.5 data-[state=active]:bg-gradient-primary data-[state=active]:text-white">
                      <span>{event}</span>
                      <Badge variant="secondary" className={`text-[10px] px-2 py-0.5 ${config.category === 'Technical' ? 'bg-technical/10 text-technical' : 'bg-non-technical/10 text-non-technical'}`}>
                        {config.category}
                      </Badge>
                    </TabsTrigger>;
                })}
                    ...
                    <Card className="border-0 shadow-card">
                      <CardHeader>
                        ...
                        <CardDescription className="text-base">
                          {event === 'Paper Showcase' && 'Present your research or project ideas with innovative solutions. File upload required for presentation materials.'}
